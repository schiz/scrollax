<?php
if ( !function_exists( 'miss_shortcode_portfolio' ) ) :
...
function miss_shortcode_portfolio( $atts ) {
    global $post;
    $temp = clone $post;
    extract(
        shortcode_atts(
    ...
    if( $except || $only ) {
        ...
        $args['tax_query'] = array(
            array(
                'taxonomy'	=> 'miss_portfolio_category',
                'field'		=> 'id',
                'terms'		=> $temp_cat,
    ...
    if( $query->have_posts() ) {
        ...
        while( $query->have_posts() ) {
            ...
            $opts = get_post_meta( $post->ID, 'miss_post_thumb_opts_options', true );
            $opts = wp_parse_args( $opts, $defaults );

            $post_t_id = get_post_thumbnail_id( $post->ID );
            $img = miss_get_thumbnail(
                array(
                    'img_id'	=> $post_t_id,
                    'width'		=> 290,
                    'height'	=> 150,
...
if ( !function_exists( 'miss_shortcode_gallery' ) ) :
...
function miss_shortcode_gallery( $atts ) {
    global $post;
    ...
    $query = new Wp_Query( $args );
    add_filter( 'the_excerpt', 'miss_password_excerpt' );
    add_filter( 'the_password_form', 'miss_gallery_passform' );
    ...
    ?>">
        ...
        while( $query->have_posts() ) {
            $query->the_post();
            get_template_part('miss_gallery', 'content');
        }
        ?>
    </div>
    <?php
    ...
    remove_filter( 'the_excerpt', 'miss_password_excerpt' );
    remove_filter( 'the_password_form', 'miss_gallery_passform' );
...
if ( !function_exists( 'miss_shortcode_photos' ) ) :
...
function miss_shortcode_photos( $atts ) {
    global $wpdb, $post;
    extract(
        shortcode_atts(
            array(
    ...
    miss_get_where_filter_params( $query_str );
    ...
    // mpdify query
    add_filter( 'posts_where' , 'miss_posts_parents_where' );
    $wp_query = new Wp_Query( $args );
    remove_filter( 'posts_where' , 'miss_posts_parents_where' );
    ...
    ?>
        ...
        <?php if( $wp_query->have_posts() ): ?>
            ...
            while( $wp_query->have_posts() ) {
                $wp_query->the_post();
                get_template_part('miss_photos', 'content');
            };
            ?>
...
if ( !function_exists( 'miss_print_widget_latposts' ) ) :

/**
 * Return Latest Posts Widget
 * @since 1.4
 */


function miss_print_widget_latposts( $atts ) {
    extract(
        shortcode_atts(
            array(
                "ppp"       => 3,
    ...
    $miss_class = 'position-left';
    if( 'right' == $align ) {
        $miss_class = 'position-right';
    }

    $args = array(
        'before_widget' => '<div class="col_1-3 ' . $miss_class. '">',
        'after_widget'  => '</div>',
        'before_title'  => '<h1>',
        'after_title'   => '</h1>'
    );
...
function miss_print_widget_twitter( $atts ) {
    extract(
        shortcode_atts(
            array(
                'title'     => '',
...
function miss_print_widget_testimonials( $atts ) {
    extract(
        shortcode_atts(
            array(
                'title'     => '',
    ...
    the_widget( 'Miss_testimonials_Widget', $params, $args );
...
function miss_print_widget_logo( $atts ) {
    extract(
        shortcode_atts(
            array(
                'title'     => '',
    ...
    the_widget( 'Miss_logos_Widget', $params, $args );
...
// logo widget shortcode
function miss_print_widget_contact( $atts ) {
    extract(
        shortcode_atts(
            array(
                'title'     => '',
    ...
    the_widget( 'Miss_contact_Widget', $params, $args );
...
// print widget
function miss_print_widget_recent_photos( $atts ) {
    extract(
        shortcode_atts(
            array(
                'ppp'       => 6,
...
function miss_button_icon($atts, $content = null) {
    ...
    $ico_path = get_template_directory_uri() . '/plugins/shortcodes/shortcodeigniter/includes/buttons/images/';
    ...
    $output .= '<div class="but-wrap'.($colour?' '.$colour:'').'"><a href="'.esc_url($url).'" class="button'.($size?' '.esc_attr($size):'' ).'"'.$blank.'><span>'.$icon_img.trim($content).'</span></a></div>';
...
function miss_shortcode_google_map($atts, $content = null) {
...
function miss_shortcode_divider( $atts, $content = null ) {
    extract( shortcode_atts( array(
		'style' => 'wide'
    ), $atts));
