<?php
function miss_shortcbuilder_photos_ppp_filter( $data ) {
...
function miss_shortcbuilder_photos_orderby_filter( $data ) {
    $data['before'] = '<legend>Ordering settings:</legend>' . $data['before'];
    return $data;
}

function miss_shortcbuilder_photos_order_filter( $data ) {
    $data['before'] = '<legend>Sort by:</legend>' . $data['before'];
    return $data;
}

// print code for selecting items in wp-admin   
function miss_admin_select_list( $opts = array() ) {
    if( empty($opts) ) {
        return false;
    }
    ...
    ?>
    <script>
        missAdmin.box = { "#<?php echo $opts['con_class']; ?>"};
    </script>
    <?php /*
...
function miss_admin_order_options( $opts = array(), $echo = true ) {
    if( empty($opts) ) {
        return false;
    }
    ...
    $output .= $before_element . miss_admin_page_option_orderby( $box_name, $options['orderby'] ) . $after_element;
        
    $output .= $before_element . miss_admin_page_option_order( $box_name, $options['order'] ) . $after_element;
...
function miss_admin_slug_options( $opts = array() ) {
    if( empty($opts) ) {
        return false;
    }    
...
function miss_admin_ppp_options( $opts = array(), $echo = true ) {
    if( empty($opts) ) {
        return false;
    }
    ...
    $output = miss_admin_page_option_ppp($box_name, $options['ppp']);
...
// order
function miss_admin_page_option_order( $box_name, $current = false ) {
    if( empty($box_name) ) {
        return false;
    }
    ...
    $options = apply_filters( 'miss_admin_page_option_order-options', $defaults );
...
// orderby
function miss_admin_page_option_orderby( $box_name, $current = false ) {
    if( empty($box_name) ) {
        return false;
    }
    ...
    $options = apply_filters( 'miss_admin_page_option_orderby-options', $defaults );
...
// ppp
function miss_admin_page_option_ppp( $box_name, $current = false ) {
    if( empty($box_name) ) {
        return false;
    }
    ...
    $options = apply_filters( 'miss_admin_page_option_ppp-options', $defaults );
