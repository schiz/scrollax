...
/**
 * shrortcode from Services block
 */

class misscomposerImServices {

	/**
	 *
	 */
	public static function im_services( $atts = null, $content = null ) {

		if( $atts == 'generator' ) {
			return array(
				'name' => __( 'Services', MISS_ADMIN_TEXTDOMAIN ),
				'base' => 'im_services',
				'icon' => 'im-icon-servicespaper',
				'category' => __('Theme Short-Codes', MISS_ADMIN_TEXTDOMAIN ),
				'params' => array(
					array(
						'heading' => __( 'Caption', MISS_ADMIN_TEXTDOMAIN ),
						'description' => __( 'Enter services block title.', MISS_ADMIN_TEXTDOMAIN ),
						'param_name' => 'caption',
						'type' => 'textfield',
						'value' => '',
					),
					array(
						...
						'description' => __( 'Enter services block tagline.', MISS_ADMIN_TEXTDOMAIN ),
						'param_name' => 'tagline',
						'type' => 'textfield',
						'value' => '',
					),
		...
		if( $services_query->have_posts() ) {
			...
			$out .= ( $atts['type'] == 'services_grid' ) ? '<div class="sc_layout ' .  $main_class . '"><div class="row-fluid">' : '<ul class="blogging ' . $main_class . '">';
			...
			while( $services_query->have_posts() ) {
				...
				$out .= ( $atts['type'] == 'services_grid' ? '<div class="' . $column_class . ' im-transform im-animate-element fade-in">' : '<li class="' . join( ' ', get_post_class( $post_class, get_the_ID() ) ) . '">' );
				if ($atts['type'] == 'services_list') {
				...
				$out .= ( $atts['type'] == 'services_grid' ? '</div>' : '</li>' );
			}; //while
			$out .= ( $atts['type'] == 'services_grid' ) ? '</div></div>' : '</ul>';
		}; //if
		wp_reset_query();
		return '<div class="sc-services ' . $class . $animation . '">' . $out . '</div>';
	}
