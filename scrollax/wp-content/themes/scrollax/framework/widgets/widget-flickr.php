 ...
 *
 */
class IrishMissW_Flickr_Widget extends WP_Widget {
	/**
	 *
	 */
    function IrishMissW_Flickr_Widget() {
		$widget_ops = array('classname' => 'miss_flickr_widget', 'description' => __( 'Pulls in images from your Flickr account', MISS_ADMIN_TEXTDOMAIN ) );
		$control_ops = array('width' => 250, 'height' => 200);
		$this->WP_Widget( 'flickr', sprintf( __( '%1$s - Flickr', MISS_ADMIN_TEXTDOMAIN ), THEME_NAME ), $widget_ops, $control_ops );
    }
	/**
	 *
	 */
    function widget($args, $instance) {	
        extract( $args );
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Photos on flickr', MISS_TEXTDOMAIN) : $instance['title'], $instance, $this->id_base);
		$tagline = apply_filters('widget_tagline', empty($instance['tagline']) ? __('Photos on flickr', MISS_TEXTDOMAIN) : $instance['tagline'], $instance, $this->id_base);
		$id = $instance['id'];
    ...
    function form($instance) {
		...
		<p><label for="<?php echo $this->get_field_id('id'); ?>"><?php _e( 'Flickr ID (<a href="http://www.idgettr.com" target="_blank">idGettr</a>):', MISS_ADMIN_TEXTDOMAIN ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id('id'); ?>" name="<?php echo $this->get_field_name('id'); ?>" type="text" value="<?php echo $id; ?>" /></p>
		<p><label for="<?php echo $this->get_field_id('number'); ?>"><?php _e( 'Number of photos:', MISS_ADMIN_TEXTDOMAIN ); ?></label>
		...
		<p><label for="<?php echo $this->get_field_id('display'); ?>"><?php _e( 'Display:', MISS_ADMIN_TEXTDOMAIN ); ?></label>
		...
		<p><label for="<?php echo $this->get_field_id('size'); ?>"><?php _e( 'Size:', MISS_ADMIN_TEXTDOMAIN ); ?></label>
		<select name="<?php echo $this->get_field_name('size'); ?>" id="<?php echo $this->get_field_id('size'); ?>" class="widefat">
		<option value="s"<?php selected($size,'square');?>><?php _e( 'Square', MISS_ADMIN_TEXTDOMAIN ); ?></option>
		<option value="t"<?php selected($size,'thumbnail');?>><?php _e( 'Thumbnail', MISS_ADMIN_TEXTDOMAIN ); ?></option>
		<option value="m"<?php selected($size,'medium');?>><?php _e( 'Medium', MISS_ADMIN_TEXTDOMAIN ); ?></option>
		</select></p>
	<?php
    }
}
