...
class missBlogging {
	 ...
	 *
	 */
	public static function blog_grid( $atts, $content = null, $code = null ) {
		if( $atts == 'generator' ) {
			$option = array(
...
//							'5' => __('Five Column', MISS_ADMIN_TEXTDOMAIN )
					...
					array(
						...
						'desc' => __( 'This will skip a number of posts at the beginning.<br /><br />Useful if you are using multiple blog shortcodes on the same page.', MISS_ADMIN_TEXTDOMAIN ),
						'id' => 'offset',
...
*/
					array(
						'name' => __('Blog Categories <small>(optional)</small>', MISS_ADMIN_TEXTDOMAIN ),
	 ...
	 *
	 */
	public static function blog_list($atts, $content = null, $code = null ) {
		if( $atts == 'generator' ) {
			$option = array( 
				'name' => __('Blog List Layout', MISS_ADMIN_TEXTDOMAIN ),
				'value' => 'blog_list',
				'options' => array(
					array(
	...
	function _blog_shortcode( $args = array() ) {
		...
		if( $blog_query->have_posts() ) {
			...
			$out .= ( $args['type'] == 'blog_grid' ) ? '<div class="sc_layout ' .  $main_class . '"><div class="row-fluid emerging_group">' : '<ul class="blogging ' . $main_class . '">';
			...
			while( $blog_query->have_posts() ) {
				...
				if ( ($spans_in_row * $row_walk) == $span_walk ) {
					$out .= '</div><!-- /.row-fluid --> <div class="row-fluid emerging_group">';
				} else {
					$out .= '';
				}
				if ( ($spans_in_row * $row_walk) == $span_walk ) {
				...
				$out .= ( $args['type'] == 'blog_grid' ? '<div class="' . $column_class . '">' : '<li class="' . join( ' ', get_post_class( $post_class, get_the_ID() ) ) . '">' );
		/*
				$out .= ( $args['type'] == 'blog_list' ) ? '<div class="' . join( ' ', get_post_class( $post_class, get_the_ID() ) ) . '">' : '<li class="' . join( ' ', get_post_class( $post_class, get_the_ID() ) ) . '">';
		*/		
			...
			$out .= ( $args['type'] == 'blog_grid' ) ? '</div></div>' : '</ul>';
		}; //if
		wp_reset_query();
		return $out;
	}
	...
	public static function _options( $class ) {
		...
		$options = array(
			'name' => __('Blog', MISS_ADMIN_TEXTDOMAIN ),
