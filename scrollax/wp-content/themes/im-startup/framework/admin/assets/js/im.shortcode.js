...
missAdmin.shortcodeFontAwesome = function() {
	var icon = jQuery("#shortcode_fontawesome #sc-fontawesome-icon"),
		previewClass = 'icon_preview',
		atts = jQuery(".shortcode_atts_fontawesome"),
		color = atts.find("#sc-fontawesome-color");
	//display preview
	icon.on('change', function() {
		jQuery("." + previewClass ).remove();

		var selector = jQuery(".shortcode_selector .miss_select"),
			container = jQuery("#shortcode_fontawesome"),
			preview = jQuery('<div class="' + previewClass + '" />');

		preview.append("<i class='fa-icon-" + jQuery(this).val() + "'></i>" );
		preview.prependTo( icon.parent().parent().parent() );
		//setTimeout( function () { jQuery(".icon_preview").remove(); }, 3000 );
	});
	//colorize
	// color.bind('live', function () {
	// 	jQuery( "." + previewClass ).css("color", jQuery(this).val() );
	// });
