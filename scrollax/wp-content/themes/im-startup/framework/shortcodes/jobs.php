...
class missJobs {
	 ...
	 *
	 */
	public static function jobs_grid( $atts, $content = null, $code = null ) {
		if( $atts == 'generator' ) {
			$option = array(
				'name' => __( 'Jobs Grid Layout', MISS_ADMIN_TEXTDOMAIN ),
				'value' => 'jobs_grid',
				'options' => array(
					array(
...
//							'5' => __('Five Column', MISS_ADMIN_TEXTDOMAIN )
					...
					array(
						...
						'desc' => __( 'This will skip a number of posts at the beginning.<br /><br />Useful if you are using multiple jobs shortcodes on the same page.', MISS_ADMIN_TEXTDOMAIN ),
						'id' => 'offset',
...
/*
					...
					array(
						'name' => __('Jobs Categories <small>(optional)</small>', MISS_ADMIN_TEXTDOMAIN ),
...
*/
		...
		return self::_jobs_shortcode( $args );
	}
	
	/**
	 *
	 */
	public static function jobs_list($atts, $content = null, $code = null ) {
		if( $atts == 'generator' ) {
			$option = array( 
				'name' => __('Jobs List Layout', MISS_ADMIN_TEXTDOMAIN ),
				'value' => 'jobs_list',
				'options' => array(
					array(
...
//							'image' => __( 'Disable Post Image', MISS_ADMIN_TEXTDOMAIN ),
	...
	function _jobs_shortcode( $args = array() ) {
		...
		if( $jobs_query->have_posts() ) {
			...
			if( $args['type'] == 'jobs_grid' ) {
				$column = ( $column > 0 || $column < 4 ) ? $column : 4;
				switch( $column ) {
	/*				case 1:
						$main_class = 'post_grid one_column';
						...
						break;
	*/				case 2:
						$main_class = 'post_grid two_column';
						$post_class = 'post_grid_module';
						$content_class = 'content';
						$img_class = 'image';
						...
						break;
					case 3:
						$main_class = 'post_grid three_column';
						$post_class = 'post_grid_module';
						$content_class = 'content';
						$img_class = 'image';
						...
						break;
					case 4:
						$main_class = 'post_grid four_column';
						$post_class = 'post_grid_module';
						$content_class = 'content';
						$img_class = 'image';
...
*/
			...
			$out .= ( $args['type'] == 'jobs_grid' ) ? '<div class="blogging ' .  $main_class . '"><div class="row-fluid">' : '<ul class="blogging ' . $main_class . '">';
			...
			while( $jobs_query->have_posts() ) {
				...
				$out .= ( $args['type'] == 'news_grid' ? '<div class="' . $column_class . '">' : '<li class="' . join( ' ', get_post_class( $post_class, get_the_ID() ) ) . '">' );
		/*
				$out .= ( $args['type'] == 'jobs_list' ) ? '<div class="' . join( ' ', get_post_class( $post_class, get_the_ID() ) ) . '">' : '<li class="' . join( ' ', get_post_class( $post_class, get_the_ID() ) ) . '">';
		*/		
				...
				$out .= ( $args['type'] == 'jobs_grid' ? '</div>' : '</li>' );
			}; //while
			$out .= ( $args['type'] == 'jobs_grid' ) ? '</div></div>' : '</ul>';
		}; //if
		wp_reset_query();
		return $out;
	}
	...
	public static function _options( $class ) {
		...
		$options = array(
			'name' => __('Jobs', MISS_ADMIN_TEXTDOMAIN ),
			'desc' => __( 'Choose which type of jobs you wish to use.<br /><br />The grid will display posts in a column layout while the list will display your posts from top to bottom.', MISS_ADMIN_TEXTDOMAIN ),
			'value' => 'jobs',
			'options' => $shortcode,
